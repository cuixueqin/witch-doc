...
tab = sym_witch[id %in% 117:127,.(Symbol,Definition,GAMS,Unit)]
knitr::kable(
tab, booktabs = TRUE,
caption = 'Knowledge symbolic terms'
)
```
...
library(data.table)
sym_witch = fread("symbols.csv",sep=",",header=T)
tab = sym_witch[id %in% 128:132,.(Symbol,Definition,GAMS)]
knitr::kable(
tab, booktabs = TRUE,
caption = 'Innovation symbolic terms'
)
```
